...
public sealed class MultiplayerHotPathAndRestoreTests
{
    ...
    [Fact]
    public void FailedSnapshotAttemptsStopAfterTwoRetries()
    {
        var scheduler = new NeonLetterSnapshotRequestScheduler();
        double interval =
            NeonLetterSnapshotRequestScheduler.RetryIntervalSeconds;

        scheduler.DeferRetry(nowSeconds: 0d);
        bool firstRetryDue = scheduler.IsDue(interval);
        scheduler.DeferRetry(nowSeconds: interval);
        bool secondRetryDue = scheduler.IsDue(interval * 2d);
        scheduler.DeferRetry(nowSeconds: interval * 2d);

        Assert.Equal(
            (3, 0, true, true, false),
            (
                scheduler.TotalAttemptCount,
                scheduler.SuccessfulSendCount,
                firstRetryDue,
                secondRetryDue,
                scheduler.IsDue(interval * 3d)));
    }

    [Fact]
    public void SuccessfulAndFailedSnapshotAttemptsShareOneRetryBudget()
    {
        var scheduler = new NeonLetterSnapshotRequestScheduler();
        double interval =
            NeonLetterSnapshotRequestScheduler.RetryIntervalSeconds;
        ...
        scheduler.DeferRetry(nowSeconds: interval);
        scheduler.RecordSuccessfulSend(nowSeconds: interval * 2d);
    ...
    [Fact]
    public void RearmingSnapshotRequestsClearsAttemptHistoryAndCompletion()
    {
        var scheduler = new NeonLetterSnapshotRequestScheduler();
        double interval =
            NeonLetterSnapshotRequestScheduler.RetryIntervalSeconds;
        ...
        scheduler.DeferRetry(nowSeconds: interval);
        scheduler.Complete();
        bool canAttemptWhileComplete = scheduler.CanAttempt;
        bool dueWhileComplete = scheduler.IsDue(interval * 2d);
    ...
    [Fact]
    public void FailedSnapshotSendDoesNotConsumeSuccessfulSendBudget()
    {
        var scheduler = new NeonLetterSnapshotRequestScheduler();

        scheduler.DeferRetry(nowSeconds: 10d);
    ...
    [Fact]
    public void SnapshotRetriesAreCappedAndCanBeRearmed()
    {
        var scheduler = new NeonLetterSnapshotRequestScheduler();
        double interval =
            NeonLetterSnapshotRequestScheduler.RetryIntervalSeconds;
        ...
        scheduler.RecordSuccessfulSend(nowSeconds: 0d);
        bool dueBeforeFirstRetry = scheduler.IsDue(interval - 0.001d);
        bool dueAtFirstRetry = scheduler.IsDue(interval);
        scheduler.RecordSuccessfulSend(nowSeconds: interval);
        scheduler.RecordSuccessfulSend(nowSeconds: interval * 2d);
        scheduler.RecordSuccessfulSend(nowSeconds: interval * 3d);
        int successfulSendCount = scheduler.SuccessfulSendCount;
        bool dueAfterMaximum = scheduler.IsDue(interval * 3d);
        scheduler.Rearm();

        Assert.Equal(
            (
                NeonLetterSnapshotRequestScheduler.MaximumSuccessfulSends,
                false,
                true,
                false,
                0,
    ...
    [InlineData(2, -1d)]
    public void SnapshotSchedulerRejectsInvalidTime(
        int operation,
        double nowSeconds)
    {
        var scheduler = new NeonLetterSnapshotRequestScheduler();
        ...
        ArgumentOutOfRangeException exception =
            Assert.Throws<ArgumentOutOfRangeException>(
                () =>
                {
                    ...
                    {
                        case 0:
                            scheduler.IsDue(nowSeconds);
                            break;
                        case 1:
                            scheduler.RecordSuccessfulSend(nowSeconds);
                            break;
                        default:
                            scheduler.DeferRetry(nowSeconds);
                            break;
                    }
                });
